...
class Dataset(torch.utils.data.Dataset):
    ...
    ----------
    img_dim : int
        the output image size; read-only
    '''
    def __init__(self, library, img_dim=512, to_gpu=False):
        '''
        Parameters
        ----------
        ...
        to_gpu : bool
            if enabled then the data will be automatically moved into the GPU;
            this may result in memory exhaustion if the images are too large
        '''
        self._library = library
        self._img_dim = img_dim
        self._hashes = list(library.get_hashes(sort_by_date=False))
        if torch.cuda.is_available() and to_gpu:
