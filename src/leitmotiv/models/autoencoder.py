...
def _decoder_block(C_in, C_out, K, batchnorm=True, is_output=False):
    ...
    if is_output:
        layers.append(('tanh', torch.nn.Tanh()))
    else:
        layers.append(('relu', torch.nn.ReLU()))
...
class Encoder(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        mu = F.hardtanh(mu, min_val=-4, max_val=4)
...
class VariationalAutoencoder(Model):
    ...
    def cost(self, data):
        ...
        l2 = torch.sum((data - reconstruction)**2, dim=[1, 2, 3])
        kl = 0.5*torch.sum((1 + 2*torch.log(sigma) - mu**2 - sigma**2),
                           dim=[1]).squeeze()
    ...
    def train(self, data):
        ...
        elbo, l2, kl = self.cost(data)
        loss = -elbo.sum()
        loss.backward()
        self.optim.step()

        return {'elbo': elbo, 'l2': l2, 'kl': kl}
